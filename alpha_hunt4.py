"""
Alpha Hunt v4 — DD-guard-aware, full backtester validation.

Key lessons from v3:
  - SL-first priority fix: sl_hit takes precedence over tp_hit
  - DD guard in real backtester blocks trades after early losses →
    smaller SL = smaller individual losses = guard triggers less = more trades
  - Current period (Apr-Jun 2026) needs 15+ trades to be meaningful
  - Steady Uptrend is structurally incompatible; accept 4/5

Phase 1: Fast vectorised scan (corrected SL-first)
Phase 2: Top-N configs validated through full bar-by-bar backtester
"""
...
PT      = 0.01
LOTS    = 0.1
LOT_VAL = PT * 100 * LOTS   # $0.10 per point at 0.1 lot
COMM    = 0.40
BALANCE = 10_000.0
...
# Alpha bar: pass 4/5 periods minimum (Steady Uptrend allowed 1 miss)
ALPHA_PF       = 1.10
ALPHA_MONTHLY  = 0
ALPHA_MAX_DD   = -1200
ALPHA_WORST_DAY= -4.0
ALPHA_MIN_TR   = 8
ALPHA_PERIODS  = 4   # must pass at least this many
...
def build_indicators(df_full):
    ...
    ph=hi.shift(1); pl=lo.shift(1); pc=cl.shift(1)
...
def build_opps(df_full, ind):
    ...
    print(f"  {len(opps):,} opportunities  "
          f"({len(opps[opps.session=='ARB'])} ARB, {len(opps[opps.session=='NYO'])} NYO)")
    return opps


# ── Fast vectorised scorer ────────────────────────────────────────────────────

def score_fast(opps, cfg):
    """Returns per-period metrics dict or None if any period has <ALPHA_MIN_TR trades."""
    s = opps.copy()

    if not cfg["use_arb"]: s = s[s["session"]=="NYO"]
    elif not cfg["use_nyo"]: s = s[s["session"]=="ARB"]
    if cfg["dow"]=="mon-thu": s = s[s["dow"]<=3]
    elif cfg["dow"]=="no-fri": s = s[s["dow"]!=4]
    if cfg["min_atr"]>0: s = s[s["h1_atr"]>=cfg["min_atr"]]
    if cfg["min_adx"]>0: s = s[s["h1_adx"]>=cfg["min_adx"]]
    if cfg["atr_exp"]:   s = s[s["atr_exp"]==1]
    if cfg["trend"]:     s = s[s["direction"]==s["h1_trend"]]

    if len(s) < ALPHA_MIN_TR*2: return None

    sl_pts = _sl(s, cfg)
    tp_pts = sl_pts * cfg["tp_mult"]
    tp_p   = s["entry"].values + s["direction"].values * tp_pts * PT
    sl_p   = s["entry"].values - s["direction"].values * sl_pts * PT
    sl_hit = np.where(s["direction"].values==1, s["day_low"].values<=sl_p,
                                                 s["day_high"].values>=sl_p)
    tp_hit = np.where(s["direction"].values==1, s["day_high"].values>=tp_p,
                                                 s["day_low"].values<=tp_p)
    win    = tp_hit & ~sl_hit
    pnl    = np.where(win, tp_pts, -sl_pts) * LOT_VAL
    pnl   -= s["spread"].values*0.5*LOT_VAL + COMM

    s2 = s.copy(); s2["pnl"]=pnl; s2["win"]=win

    results = {}
    passes  = 0
    for label, start, end in PERIODS:
        p = s2[(s2["date_dt"]>=start) & (s2["date_dt"]<=end)]
        if len(p) < ALPHA_MIN_TR: continue
        mo_len = (pd.to_datetime(end)-pd.to_datetime(start)).days/30
        wp = p[p["win"]]["pnl"].sum()
        lp = p[~p["win"]]["pnl"].sum()
        pf = wp/-lp if lp<0 else 99.0
        mo_avg = p["pnl"].sum()/mo_len

        cum  = np.cumsum(p["pnl"].values)
        peak = np.maximum.accumulate(cum)
        max_dd = float((cum-peak).min())

        day_pnl   = p.groupby("date_int")["pnl"].sum()
        worst_day = day_pnl.min()/BALANCE*100

        ok = (pf>=ALPHA_PF and mo_avg>=ALPHA_MONTHLY
              and max_dd>=ALPHA_MAX_DD and worst_day>=ALPHA_WORST_DAY)
        if ok: passes += 1
        results[label] = {"trades":len(p),"pf":round(pf,3),
...
def _sl(s, cfg):
    m = cfg["sl_mode"]
    if m=="fixed":
        return np.full(len(s), float(cfg["sl_fixed"]))
    elif m=="atr":
        return np.clip(s["h1_atr"].values*cfg["sl_atr_m"]/PT,
                       cfg["sl_min"], cfg["sl_max"])
    else:  # range
        return np.clip(s["range_pts"].values*0.5,
                       cfg["sl_min"], cfg["sl_max"])


# ── Full backtester validation ────────────────────────────────────────────────

class ValidateStrat(Strategy):
    def __init__(self, cfg, bal, arb_r, nyo_r, h1_atr_s, ind):
        ...
        self.arb_ranges=arb_r; self.nyo_ranges=nyo_r
        self.h1_atr=h1_atr_s; self.ind=ind
    ...
    def next(self, i, df):
        ...
        if self.cfg["use_arb"] and 8<=hour<10 and not self._arb_done and today in self.arb_ranges.index:
            ...
            if 500<=rng<=9000:
                if self.cfg["trend"] and trnd!=0:
                    pass
                sl=_sl_pts(); tp=int(sl*self.cfg["tp_mult"])
    ...
    def _c(self): self._in_trade=False; self._dir=self._sl=self._tp=None; return "close"


def validate_full(cfg, df_full, arb_r, nyo_r, ind):
    """Run full bar-by-bar backtester on all 5 periods."""
    h1_atr_s = ind["atr"]
    results = {}
    for label, start, end in PERIODS:
        df = df_full.loc[start:end].copy()
        iv = ind.reindex(df.index, method="ffill")
        strat = ValidateStrat(cfg, BALANCE, arb_r, nyo_r,
                              iv["atr"], iv)
...
def print_validation(cfg, results):
    ...
    sess = "ARB+NYO" if cfg["use_arb"] and cfg["use_nyo"] else ("ARB" if cfg["use_arb"] else "NYO")
    extras=[]
...
def scan(top_n=15):
    ...
    # Slice period opps
    for label, start, end in PERIODS:
        n = ((opps["date_dt"]>=start)&(opps["date_dt"]<=end)).sum()
        print(f"  {label}: {n} opps")

    # Grid — focused on smaller SL to beat DD guard
    cfgs = []
    ...
    print(f"\nPhase 1 — scanning {len(cfgs):,} configs …")
    t0=time.time()

    scored=[]
    for cfg in cfgs:
        r = score_fast(opps, cfg)
        if r is None: continue
        passes = r["_passes"]
        avg_mo = np.mean([v["monthly_avg"] for v in r.values()
                          if isinstance(v, dict) and "monthly_avg" in v])
        min_pf = min((v["pf"] for v in r.values()
                      if isinstance(v,dict) and "pf" in v), default=0)
    ...
    print(f"Phase 1 done in {elapsed:.1f}s — {len(scored):,} scored, "
          f"{len(alphas)} pass {ALPHA_PERIODS}+ periods")

    # Deduplicate: same passes+avg_mo → keep first
    seen=set(); top=[]
    for s in scored:
        key=(s["passes"],round(s["avg_mo"],0),round(s["min_pf"],2))
        if key not in seen: seen.add(key); top.append(s)
    ...
    for s in top[:top_n]:
        ...
        full_avg_mo = np.mean([vr[lbl]["monthly"] for lbl,_,_ in PERIODS
                               if vr.get(lbl) and vr[lbl]])
        validated.append({"cfg":s["cfg"],"passes":full_passes,
                          "avg_mo":full_avg_mo,"vr":vr})

    validated.sort(key=lambda x:(x["passes"],-x["avg_mo"]), reverse=True)

    print(f"\n{'='*65}")
    print(f"  ALPHA HUNT v4 — FULL VALIDATION RESULTS")
