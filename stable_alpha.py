...
# ── STABILIZED: portfolio vol-target + YTD throttle ───────────────────────────
# Goal: compress year-to-year variance. As each calendar year's gains accumulate,
# scale position size DOWN so no single year runs away (e.g. 2025 metals melt-up).
print("\n" + "=" * 70)
print("  STABILIZED: 30% port vol + YTD throttle (target ~consistent annual band)")
print("=" * 70)

PORT_VOL = 0.30   # lower base vol than full-power 55% → tighter band
PORT_CAP = 5.0
TARGET   = 0.40   # annual return above which we start throttling hard

port_raw = best_port["net"]
idx      = best_port["idx"]

def ytd_throttle(ytd):
    """Scale factor as a function of year-to-date return.
    Below target: full size. Above: ramp down aggressively so the year caps out."""
    if ytd <= 0.20:  return 1.00
    if ytd <= 0.40:  return 1.00 - 0.50 * (ytd - 0.20) / 0.20   # 1.00 → 0.50
    if ytd <= 0.70:  return 0.50 - 0.35 * (ytd - 0.40) / 0.30   # 0.50 → 0.15
    return 0.10                                                  # hard floor

# Portfolio vol-target (slower 20-day window for stability)
rv_p  = port_raw.rolling(20).std() * np.sqrt(ann)
sc_v  = (PORT_VOL / rv_p.replace(0, np.nan)).clip(upper=PORT_CAP).fillna(1.0).shift(1).fillna(1.0)

# Walk forward applying YTD throttle (resets each calendar year, no lookahead)
years_arr = idx.year.values
port_vals = np.zeros(len(port_raw))
raw_vals  = port_raw.values
vsc_vals  = sc_v.values
cur_year  = years_arr[0]
ytd_eq    = 1.0   # year-to-date equity (resets each Jan)
for i in range(len(raw_vals)):
    if years_arr[i] != cur_year:
        cur_year = years_arr[i]; ytd_eq = 1.0
    th = ytd_throttle(ytd_eq - 1.0)            # throttle from YESTERDAY's ytd (no lookahead)
    daily = raw_vals[i] * vsc_vals[i] * th
    port_vals[i] = daily
    ytd_eq *= (1.0 + daily)
...
annvals = np.array([v*100 for v in by.values])
...
print(f"  Annual avg {annvals.mean():.1f}%  std {annvals.std():.1f}%  "
      f"min {annvals.min():.1f}%  max {annvals.max():.1f}%")
...
ax1.plot(idx, eq.values, lw=2.8, color="gold", alpha=0.95,
         label=f"STABILIZED (30% vol + YTD throttle)  Sh {sh:.2f}  CAGR {cagr*100:.1f}%  DD {dd*100:.1f}%")
ax1.axvline(idx[sp], color="red", ls="--", alpha=0.4, lw=1.2, label="IS/OOS split")
ax1.set_yscale("log")
ax1.legend(fontsize=7.5, loc="upper left")
ax1.grid(alpha=0.25)
ax1.set_title(f"Stable Alpha: 4 Premia + YTD throttle (variance-controlled)\n"
              f"Sh {sh:.2f}  CAGR {cagr*100:.1f}%  maxDD {dd*100:.1f}%  "
              f"annual {annvals.mean():.0f}%±{annvals.std():.0f}%")
...
plt.tight_layout()
plt.savefig("stable_alpha.png", dpi=130)
print(f"\n  Saved → stable_alpha.png")
